...
def connect_db():
    try:
        conn = mysql.connector.connect(user='root', password='', host='localhost', database='yizhi', charset="utf8")
    except Exception, e:
...
def index():
    if g.user:
        g.cursor.execute("""select * from yizhi_posts where username='%s';""" % g.user['username'])
    ...
    if checklogin(username,password):
        ...
        g.cursor.execute("""select * from yizhi_posts where username='%s';""" % username)
        result = g.cursor.fetchall()
        blogs = []
        for r in result:
            temp = {}
...
def found():
    ...
    for r in result:
        ...
        temp['summary'] = r[3][0:100]+" ..."
        blogs.append(temp)
    return render_template('found.html', blogs = blogs)

#以图像墙的形式展示用户
@app.route('/users')
def users():
    #g.cursor.execute("""select * from yizhi_users""")
    #result = g.cursor.fetchall()
    #users = []
    #for r in result:
     #   temp = []
...
def register():
    ...
    if request.method == 'POST':
        ...
        if username == "" or password == "" or repassword == "" or email == "":
            ...
            g.cursor.execute("""select username from yizhi_users where username='%s';""" % user['username'])
            result = g.cursor.fetchall()
            if result != []:
                flash(u'用户名已存在，请重新尝试。')
                return render_template('register.html', user = user)
            #check if email has already existed
            g.cursor.execute("""select email from yizhi_users where username='%s';""" % user['email'])
            result = g.cursor.fetchall()
            if result != []:
            ...
            g.cursor.execute("""select url from yizhi_users where url='%s';""" % url)
            result = g.cursor.fetchall()
            if result != []:
                url = create_token()
            #all is well
            user['password'] = md5encrypt(user['password'])
            sql = """insert into `yizhi_users` (`username`,`password`,`regtime`,`email`,`url`,`private`) values ('%s','%s','%s','%s','%s',%d);"""\
            ...
            sql = """insert into `yizhi_posts` (`username`,`pubtime`,`content`) values ('%s','%s','%s');"""\
                        % (user['username'],getCurrentTime(),u"今天注册了忆志，开始记录自己的心情。")
            g.cursor.execute(sql)
            session['logged_in'] = True
            session['username'] = username
            g.user = user
...
def checklogin(username,password):
    g.cursor.execute("""select username,password from yizhi_users where username='%s';""" % username)
    result = g.cursor.fetchall()
    if result == []:
        return False
...
def getCurrentUser():
    if 'logged_in' not in session or 'username' not in session:
        return None
    g.cursor.execute("""select * from yizhi_users where username='%s';""" % session['username'])
    result = g.cursor.fetchall()
...
def avatar_upload():
    ...
    if afile and allowed_file(afile.filename):
        ...
        sql = ("""update `yizhi_users` set `avatar`='%s' where `username`='%s';"""\
...
def personal_update():
    ...
    email = request.form['email']
    sql = ("""update `yizhi_users` set `nickname`='%s', `solo`='%s', `email`='%s' where `username`='%s';"""\
...
def privacy_update():
    privacy = int(request.form['privacy'])
    sql = ("""update `yizhi_users` set `private`=%d where `username`='%s';"""\
...
@require_login
def myhome(url):
    sql = """select * from `yizhi_posts` where `username`='%s' order by `id` desc limit 1;"""\
                    % (g.user['username'])
    g.cursor.execute(sql)
    result = g.cursor.fetchall()
    if result[0] == ():
